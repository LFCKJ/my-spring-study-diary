   ...
   public StudyLog save(StudyLog studyLog){
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
                ...
                ps.setString(5, studyLog.getStudyTime());
                ps.setString(6, String.valueOf(studyLog.getStudyDate()));
                return ps;
        }, keyHolder);
    ...
    public StudyLog update(StudyLog studyLog){
        String sql = """
                UPDATE study_logs 
                SET title =?, content=?,category=?,understanding=?,study_time=?,study_date=? WHERE id = ?
                """;
